...
class PageController extends Controller
{
    ...
    {
        $score = Score::all();
        return view('page.thithu', compact('score'));
    }
    ...
    public function getLessonFirst($course_id){
        ...
        $id_user = Auth::id();
        $mid_test = Mid_Test::where('id_course', $course_id)->orderby('id')->first();

        if($mid_test == null){
            $id_midtest_first = 0;
        }
        else{
            $id_midtest_first = $mid_test->id;
        }

        $mid_test_result = Mid_Test_Result::where('id_midtest',$id_midtest_first)
                                            ->where('id_user', $id_user)->count('id');
                                   
        $end_test = End_Test::where('id_course', $course_id)->orderby('id')->first();
        if($end_test == null){
            $id_endtest_first = 0;
        }
        else{
            $id_endtest_first = $end_test->id;
        }
    ...
    public function getLesson($lesson_id){
        ...
        $inputs = $user_lesson->pluck('id_lesson')->toArray();
        $count_student = Register::where('id_course', $course_id)->count('id_user');
        $recomment = Re_comment::all();
 
        return view('page.lesson', compact('chitietcourse', 'lesson', 'lessonshow', 'count_student', 'user_lesson', 'check_lesson', 'inputs', 'recomment'));
    }
    ...
    public function getSearch(Request $req){
        $course = Course::where('title', 'LIKE', '%' .$req->key. '%')->get();
