...
class BaiTapController extends Controller
{
    ...
    public function getEx1($lesson_id){
        ...
        if($ex1 == null){
            $ex1_id = 0;
            return redirect()->back()->with('message', 'Chưa Có Bài Tập nào');
        }
        else{
            $ex1_id = $ex1->id;
        }
        $id_ex1 = $ex1_id + 1;
        $check = Ex1::where('id', $id_ex1)->first();

        $user_id = Auth::user()->id;
        $join_table = DB::table('ex1')
            ->join('submit_ex1','ex1.id','=','submit_ex1.id_ex1')
            ->where(['ex1.id' => $ex1_id, 'submit_ex1.id_user' => $user_id])
            ->select('submit_ex1.answer', 'submit_ex1.id', 'submit_ex1.id_ex1', 'ex1.file', 'submit_ex1.result as result')
            ->orderBy('id')->first();
        return view('page.baitap.ex1', compact('ex1', 'join_table', 'number_question', 'id_ex1', 'count_question', 'check'));
    }
    ...
    public function getEx2($lesson_id){
        ...
        $id_ex2 = $ex2_id + 1;
        $check = Ex2::where('id', $id_ex2)->first();
        $user_id = Auth::user()->id;
        $join_table = DB::table('ex2')
            ->join('submit_ex2','ex2.id','=','submit_ex2.id_ex2')
            ->where(['ex2.id' => $ex2_id, 'submit_ex2.id_user' => $user_id])
            ->select('submit_ex2.answer', 'submit_ex2.id', 'submit_ex2.id_ex2', 'ex2.file', 'submit_ex2.result as result')
            ->orderBy('id')->first();
        return view('page.baitap.ex2', compact('ex2', 'join_table', 'number_question', 'id_ex2', 'count_question', 'check'));
    }
    ...
    public function getEx3($lesson_id){
        ...
        $id_ex3 = $ex3_id + 1;
        $check = Ex3::where('id', $id_ex3)->first();
        $user_id = Auth::user()->id;
        $join_table = DB::table('ex3')
            ->join('submit_ex3','ex3.id','=','submit_ex3.id_ex3')
            ->where(['ex3.id' => $ex3_id, 'submit_ex3.id_user' => $user_id])
            ->select('submit_ex3.answer', 'submit_ex3.id', 'submit_ex3.id_ex3', 'ex3.Content', 'submit_ex3.result as result')
            ->orderBy('id')->first();
        return view('page.baitap.ex3', compact('ex3', 'join_table', 'number_question', 'id_ex3', 'count_question', 'check'));
    }
    ...
    public function getEx4($lesson_id){
        ...
        $join_table = DB::table('ex4')
            ...
            ->select('submit_ex4.answer', 'submit_ex4.id', 'submit_ex4.id_ex4', 'ex4.file', 'submit_ex4.result as result')
            ->orderBy('id')->first();
            // dd($ex4_id);
